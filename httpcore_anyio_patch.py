...
async def _connect_tcp_via_socket(
    ...
    Docker's 127.0.0.11 nameserver from subprocesses. Connects via a
    raw socket in a thread (bypasses async resolver) then wraps it.
    """
    import asyncio
    ...
    sock = await loop.run_in_executor(None, _make_sock)

    # Use asyncio streams wrapped minimally to guarantee ByteStream compatibility
    reader, writer = await asyncio.open_connection(sock=sock)

    class _FallbackStream(anyio.abc.ByteStream):
        ...
        def extra(self, attribute, default=None):
            if attribute == anyio.abc.SocketAttribute.raw_socket:
                return sock
            if attribute == anyio.abc.SocketAttribute.local_address:
