...
# ---------------------------------------------------------------------------
# DNS workaround for JupyterHub/Docker environments where getaddrinfo() fails
# for subprocesses even on raw IPs (Docker's 127.0.0.11 DNS is unreachable).
# We patch socket.getaddrinfo to fall back to a direct IP tuple when DNS
# resolution fails, since raw TCP connections work fine in this environment.
# ---------------------------------------------------------------------------
...
def _patched_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
    try:
        ...
        # If DNS fails and host looks like a raw IPv4 address, return a
        # synthetic result so httpcore can proceed with a direct TCP connect.
        import re
        if re.match(r"^\d{1,3}(\.\d{1,3}){3}$", str(host)):
            logger.debug(
                "getaddrinfo failed for %s:%s — using raw IP fallback", host, port
            )
            return [
                (_socket.AF_INET, _socket.SOCK_STREAM, 6, "", (host, int(port or 0))),
                (_socket.AF_INET, _socket.SOCK_DGRAM,  17, "", (host, int(port or 0))),
            ]
        raise
